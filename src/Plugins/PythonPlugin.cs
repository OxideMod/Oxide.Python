...
namespace Oxide.Core.Python.Plugins
{
    ...
    public class PythonPlugin : Plugin
    {
        ...
        {
            if (Config == null || Class == null)
            {
                return;
            }

            if (!PythonEngine.Operations.ContainsMember(Class, "Config"))
            {
                return;
            }

            PythonDictionary configtable = PythonEngine.Operations.GetMember<PythonDictionary>(Class, "Config");
            if (configtable != null)
            {
        ...
        public override void Load()
        {
            ...
            ScriptSource source = PythonEngine.CreateScriptSourceFromString(code, Path.GetFileName(Filename), SourceCodeKind.Statements);
            CompiledCode compiled = source.Compile();
            compiled.Execute(Scope);
            if (!Scope.ContainsVariable(Name))
            {
                throw new Exception("Plugin is missing main class");
            }
            ...
            if (!PythonEngine.Operations.ContainsMember(Class, "Title") || PythonEngine.Operations.GetMember<string>(Class, "Title") == null)
            {
                throw new Exception("Plugin is missing title");
            }

            if (!PythonEngine.Operations.ContainsMember(Class, "Author") || PythonEngine.Operations.GetMember<string>(Class, "Author") == null)
            {
                throw new Exception("Plugin is missing author");
            }

            if (!PythonEngine.Operations.ContainsMember(Class, "Version") || PythonEngine.Operations.GetMember(Class, "Version").GetType() != typeof(VersionNumber))
            {
            ...
            if (PythonEngine.Operations.ContainsMember(Class, "Description"))
            {
                Description = PythonEngine.Operations.GetMember<string>(Class, "Description");
            }

            if (PythonEngine.Operations.ContainsMember(Class, "ResourceId"))
            {
                ResourceId = PythonEngine.Operations.GetMember<int>(Class, "ResourceId");
            }
            ...
            IList<string> globals = PythonEngine.Operations.GetMemberNames(Class);
            foreach (string name in globals)
            {
                object func;
                if (!PythonEngine.Operations.TryGetMember(Class, name, out func) || !PythonEngine.Operations.IsCallable(func))
                {
                    continue;
                }

                Globals.Add(name, func);
                if (!PythonEngine.Operations.ContainsMember(func, "__dict__"))
                {
                    continue;
                }

                PythonDictionary dict = PythonEngine.Operations.GetMember<PythonDictionary>(func, "__dict__");
                if (dict.ContainsKey("isCommand"))
                {
                    string[] names = ((IList<object>)dict["name"]).Cast<string>().ToArray();
                    string[] perms = ((IList<object>)dict["permission"]).Cast<string>().ToArray();
                    if (names.Length == 0)
                    {
        ...
        {
            object func;
            if (!Globals.TryGetValue(hookname, out func))
            {
                return null;
            }
            ...
            {
                string message = $"Failed to call {hookname} ({e.GetType().Name}: {e.Message}){Environment.NewLine}{e.StackTrace}";
